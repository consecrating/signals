...
class FnOSP_Signal_Engine {
	...
	public function evaluate( array $snapshot, array $opts = array() ) {
		...
		if ( $is_fno ) {
			...
			// Resolve a single expiry timestamp so the pricing DTE and the
			// displayed expiry date are always derived from the same source.
			if ( isset( $opts['dte'] ) ) {
				$dte       = max( 1, (int) $opts['dte'] );
				$expiry_ts = time() + $dte * 86400; // Honour an explicit caller override.
			} else {
				$expiry_ts = $this->next_expiry_ts( $snapshot['instrument'] );
				$dte       = max( 1, (int) ceil( ( $expiry_ts - time() ) / 86400 ) );
			}
	...
	// ---------------------------------------------------------------------
	private function build_option_plan( $s, $direction, $strike, $type, $dte, $live = 0.0, $expiry_ts = 0 ) {
		$type = ( 'PE' === $type ) ? 'PE' : 'CE';
		$ltp  = (float) $s['ltp'];
		...
		// Moneyness by distance to the strike, using half a strike-step as the
		// ATM band (the exchange convention) rather than an ATR-relative band.
		$step = $this->strike_step( $s['instrument'], $ltp );
		$diff = $ltp - $strike;
		if ( abs( $diff ) <= $step / 2 ) {
			$moneyness = 'ATM';
		} elseif ( $is_call ) {
			$moneyness = $diff > 0 ? 'ITM' : 'OTM';
		} else {
			$moneyness = $diff < 0 ? 'ITM' : 'OTM';
		}
		...
		return array(
			...
			'dte'           => $dte,
			'expiry_date'   => $expiry_ts > 0 ? gmdate( 'd M Y', $expiry_ts ) : $this->get_expiry_date_str( $dte, $s['instrument'] ),
	...
	private function apply_risk_filters( $direction, $s ) {
		...
		if ( 'BUY' === $direction ) {
			...
			$near = 0.2 * max( 0.0001, (float) $s['atr'] );
			if ( $s['ltp'] < $s['prev_ohlc']['high'] && ( $s['prev_ohlc']['high'] - $s['ltp'] ) <= $near ) {
				$factors[] = sprintf(
					'Trading just below prior-day resistance (₹%s) — confirm a breakout before entry.',
					number_format_i18n( $s['prev_ohlc']['high'], 2 )
				);
			}
	...
	private function build_option_strategy( $direction, $s, $confidence ) {
		...
		// Use the instrument's real days-to-expiry so the estimate reflects the
		// actual time value (weekly vs monthly) rather than a fixed assumption.
		$t       = max( 1, $this->days_to_next_expiry( $s['instrument'] ) ) / 365;
	...
	/**
	 * Timestamp (UTC epoch) of the next F&O expiry for an instrument.
	 *
	 * Computed dynamically from the current exchange rules instead of a
	 * hardcoded list (which goes stale):
	 *   - Expiry weekday: NSE indices settle on Tuesday, BSE (SENSEX) on Thursday.
	 *   - Weekly expiries exist only for each exchange's benchmark
	 *     (SEBI, Nov-2024): NSE -> NIFTY, BSE -> SENSEX. Every other index is
	 *     monthly-only (last expiry-weekday of the month).
	 * Expiry cut-off is 15:30 IST on the expiry day.
	 * Adjust the weekday map here if NSE/BSE revise their settlement day.
	 *
	 * @param string $instrument Instrument.
	 * @return int Unix timestamp of the next expiry.
	 */
	private function next_expiry_ts( $instrument ) {
		$instrument = strtoupper( $instrument );

		$is_bse      = in_array( $instrument, array( 'SENSEX', 'SENSEX50', 'BANKEX' ), true );
		$iso_weekday = $is_bse ? 4 : 2; // ISO-8601: Mon=1 ... Sun=7 (Tue=2, Thu=4).
		$has_weekly  = in_array( $instrument, array( 'NIFTY', 'NIFTY50', 'SENSEX' ), true );
		...
		$candidates = array();
		if ( $has_weekly ) {
			// Next several weekly expiries (this weekday, then +7 days each).
			$base = $this->weekday_on_or_after( clone $now, $iso_weekday );
			for ( $i = 0; $i < 8; $i++ ) {
				$candidates[] = ( clone $base )->modify( '+' . ( $i * 7 ) . ' days' );
			}
		} else {
			// Last expiry-weekday of this month + the following months.
			$month = ( clone $now )->modify( 'first day of this month' );
			for ( $i = 0; $i < 6; $i++ ) {
				$candidates[] = $this->last_weekday_of_month( ( clone $month )->modify( "+$i months" ), $iso_weekday );
			}
		}

		foreach ( $candidates as $c ) {
			$c->setTime( 15, 30, 0 );
			if ( $c->getTimestamp() > $now->getTimestamp() ) {
				return $c->getTimestamp();
			}
		}
		return $now->getTimestamp() + 7 * 86400;
	}

	/**
	 * Advance a date (in place) to the given ISO weekday on or after it.
	 *
	 * @param DateTime $d           Date.
	 * @param int      $iso_weekday ISO weekday (Mon=1 ... Sun=7).
	 * @return DateTime
	 */
	private function weekday_on_or_after( DateTime $d, $iso_weekday ) {
		$diff = ( $iso_weekday - (int) $d->format( 'N' ) + 7 ) % 7;
		return $d->modify( "+$diff days" );
	}

	/**
	 * Last occurrence of an ISO weekday within the month of the given date.
	 *
	 * @param DateTime $any_day_in_month Any day in the target month.
	 * @param int      $iso_weekday      ISO weekday (Mon=1 ... Sun=7).
	 * @return DateTime
	 */
	private function last_weekday_of_month( DateTime $any_day_in_month, $iso_weekday ) {
		$last = ( clone $any_day_in_month )->modify( 'last day of this month' );
		$diff = ( (int) $last->format( 'N' ) - $iso_weekday + 7 ) % 7;
		return $last->modify( "-$diff days" );
	}

	/**
	 * Days to the next F&O expiry (minimum 1).
	 *
	 * @param string $instrument Instrument.
	 * @return int
	 */
	private function days_to_next_expiry( $instrument ) {
		$ts = $this->next_expiry_ts( $instrument );
		return max( 1, (int) ceil( ( $ts - time() ) / 86400 ) );
	}

	/**
	 * Human-readable expiry date. Prefers the instrument's real calendar;
	 * falls back to now + $dte days only when no instrument is known.
	 *
	 * @param int    $dte        Days to expiry (fallback only).
	 * @param string $instrument Instrument (optional).
	 * @return string e.g. "28 Jul 2026".
	 */
	private function get_expiry_date_str( $dte, $instrument = '' ) {
		if ( '' !== $instrument ) {
			return gmdate( 'd M Y', $this->next_expiry_ts( $instrument ) );
		}
		return gmdate( 'd M Y', time() + max( 1, (int) $dte ) * 86400 );
	}
