...
register_deactivation_hook(
	...
	function () {
		...
		// Clear watchdog rapid-scan cron.
		if ( class_exists( 'FnOSP_Watchdog' ) ) {
			$ts = wp_next_scheduled( FnOSP_Watchdog::CRON_RAPID );
			while ( $ts ) {
				wp_unschedule_event( $ts, FnOSP_Watchdog::CRON_RAPID );
				$ts = wp_next_scheduled( FnOSP_Watchdog::CRON_RAPID );
			}
		}
		flush_rewrite_rules();
