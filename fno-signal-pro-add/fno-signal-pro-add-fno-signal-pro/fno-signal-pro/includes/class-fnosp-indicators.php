...
class FnOSP_Indicators {
	 ...
	 * @return float
	 */
	public static function bs_gamma( $s, $k, $t, $r, $sigma ) {
		$t     = max( $t, 1 / 3650 );
		$sigma = max( $sigma, 0.0001 );
		$sqrt_t = sqrt( $t );
	 ...
	 * @param float  $r     Rate.
	 * @param float  $sigma IV (decimal).
	 * @return float Theta per day (typically negative for long options).
	 */
	public static function bs_theta( $type, $s, $k, $t, $r, $sigma ) {
		$t      = max( $t, 1 / 3650 );
		$sigma  = max( $sigma, 0.0001 );
		$sqrt_t = sqrt( $t );
		$d1     = ( log( $s / $k ) + ( $r + $sigma * $sigma / 2 ) * $t ) / ( $sigma * $sqrt_t );
		...
		if ( 'PE' === strtoupper( $type ) ) {
			$theta = $term1 + $r * $k * $disc * self::norm_cdf( -$d2 );
		} else {
			$theta = $term1 - $r * $k * $disc * self::norm_cdf( $d2 );
		}
		// Convert from annualized to per-day.
		return $theta / 365;
	}
	 ...
	 * @return float
	 */
	public static function norm_pdf( $x ) {
