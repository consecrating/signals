...
class FnOSP_Rest_Api {
	...
	public function get_signal( WP_REST_Request $req ) {
		...
		if ( $has_opt ) {
			...
			// Only pin the DTE when the caller supplied one (or an expiry date).
			// Otherwise let the engine use the instrument's real expiry calendar
			// instead of a flat 7-day assumption.
			if ( $dte ) {
				$gen_opts['dte'] = max( 1, (int) $dte );
			}
